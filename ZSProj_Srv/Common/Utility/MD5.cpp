...
char *Cmd5::EncodeBytesA(char *pszOutput, UCHAR *pBytes, UINT uBytesLen)
{
    if (!pszOutput|| !pBytes || 0 == uBytesLen)
    {
        return NULL;
    }
    ...
    MD5Init (&context);
    MD5Update (&context, pBytes, uBytesLen);
    ...
    for (i = 0; i < 16; i++)
    {
        ...
        if (i < 15)
        {
            sprintf(&(szMD5[2*i+1]),"%02x",(unsigned char)(digest[i]<<4));
        }
    }
    
    memcpy(pszOutput, szMD5, 32);
