...
CNTService * GetService() { return gpTheService; }
...
CNTService :: CNTService(LPCTSTR lpServiceName, LPCTSTR lpDisplayName, LPCTSTR lpDesc)
    ...
    if( OsIsWin95() ) {	//!! TCW MOD - code added to install as Win95 service
        ...
        if( schSCManager ) {
            ...
            if( schService ) 
            {
                ...
                if(!ChangeServiceConfig2(schService,SERVICE_CONFIG_FAILURE_ACTIONS,&failure_action))
                {
                    _tprintf(TEXT("Set Service Restart failed\n"));
                }

                SERVICE_DESCRIPTION server_desc;
                server_desc.lpDescription = (LPTSTR)m_lpDesc;
                
                if(!ChangeServiceConfig2(schService,SERVICE_CONFIG_DESCRIPTION,&server_desc))
                {
                    _tprintf(TEXT("Set Service Desc failed\n"));
                }
            ...
            else 
            {
                ...
                MessageBox(NULL, szErr, TEXT("Prompt"), MB_ICONSTOP);
            }
...
BOOL CNTService :: RemoveService() {
    ...
    if( OsIsWin95() ) {	//!! TCW MOD - code added to install as Win95 service
        ...
        if( schSCManager ) {
            ...
            TCHAR szErr[256];
            MessageBox(NULL, GetLastErrorText(szErr,256), TEXT("Prompt"), MB_OK);
        }
...
LPTSTR CNTService :: GetLastErrorText( LPTSTR lpszBuf, DWORD dwSize ) {
    ...
    {
        lpszTemp[_tcslen(lpszTemp)-2] = TEXT('\0');  //remove cr/nl characters
...
void CNTService :: RegisterApplicationLog( LPCTSTR lpszFileName, DWORD dwTypes ) {
    ...
    if( ::RegCreateKey(HKEY_LOCAL_MACHINE, szKey, &hKey) == ERROR_SUCCESS ) {
        lRet =	::RegSetValueEx(
            ...
            (WORD)(_tcslen(lpszFileName) + 1)	// size of value data
            );
